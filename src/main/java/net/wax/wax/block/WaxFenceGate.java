...
public class WaxFenceGate extends FenceGateBlock {
    public WaxFenceGate(Settings settings) {
        super(settings);
    }
    ...
    public void ChangeHeatValue(BlockState state, WorldAccess world, BlockPos pos, int HeatValue) {
        ...
        if (state.get(HEAT) >= HEAT.getValues().size() - 1) {
            if (this.getDefaultState() == ModBlocks.WAX_FENCE_GATE_BLACK.getDefaultState()) {
